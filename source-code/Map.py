...
class Map:
    ...
    def __init__(self, size):
        ...
        for x, y in christmas_coords:
            self.obstacles.add(Tree((x, y, xmastree_w, xmastree_h), img_file='assert/tree/christmas_tree.png'))
        ...
        for x, y in palm_coords:
            self.obstacles.add(Tree((x, y, palmtree_w, palmtree_h), img_file='assert/tree/palm_tree.png'))

        brokentree_w, brokentree_h = 40, 40
        broken_coords = [(120, 100), (242, 300), (418, 111)]
        for x, y in broken_coords:
            self.obstacles.add(Tree((x, y, brokentree_w, brokentree_h), img_file='assert/tree/broken_tree.png'))

        mosstree_w, mosstree_h = 75, 75
        moss_coords = [(1100, 650)]
        for x, y in moss_coords:
            self.obstacles.add(Tree((x, y, mosstree_w, mosstree_h), img_file='assert/tree/moss_tree.png'))

        red_crystal_w, red_crystal_h = 30, 30
        red_crystal_coords = [(780, 450)]
        for x, y in red_crystal_coords:
            self.obstacles.add(Rock((x, y, red_crystal_w, red_crystal_h), img_file='assert/rock/red_crystal.png'))
